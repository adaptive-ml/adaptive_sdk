...
from .custom_fields import AbcampaignFields, CompletionConnectionFields, CompletionFields, CompletionGroupDataConnectionFields, DatasetFields, EvaluationJobFields, MetaObjectFields, MetricFields, ModelFields, SystemPromptTemplateFields, TeamFields, TrainingJobFields, UseCaseFields, UserFields
from .input_types import AbCampaignFilter, CursorPageInput, ListCompletionsFilterInput, ModelFilter, OrderPair, UseCaseFilter
...
class Query:
    ...
    def ab_campaigns(cls, filter: AbCampaignFilter) ->AbcampaignFields:
        ...
        return AbcampaignFields(field_name='abCampaigns', arguments=
            cleared_arguments)
    ...
    def ab_campaign(cls, id_or_key: str) ->AbcampaignFields:
        ...
        cleared_arguments = {key: value for key, value in arguments.items() if
            value['value'] is not None}
        return AbcampaignFields(field_name='abCampaign', arguments=
            cleared_arguments)

    @classmethod
    def datasets(cls, use_case: str) ->DatasetFields:
        arguments: Dict[str, Dict[str, Any]] = {'useCase': {'type':
            'IdOrKey!', 'value': use_case}}
        cleared_arguments = {key: value for key, value in arguments.items() if
            value['value'] is not None}
    ...
    def evaluation_job(cls, id: Any) ->EvaluationJobFields:
        ...
        cleared_arguments = {key: value for key, value in arguments.items() if
            value['value'] is not None}
        return EvaluationJobFields(field_name='evaluationJob', arguments=
            cleared_arguments)

    @classmethod
    def completions(cls, filter: ListCompletionsFilterInput, page:
        CursorPageInput, order: OrderPair) ->CompletionConnectionFields:
        arguments: Dict[str, Dict[str, Any]] = {'filter': {'type':
            'ListCompletionsFilterInput!', 'value': filter}, 'page': {
            'type': 'CursorPageInput!', 'value': page}, 'order': {'type':
            'OrderPair!', 'value': order}}
        cleared_arguments = {key: value for key, value in arguments.items() if
            value['value'] is not None}
        return CompletionConnectionFields(field_name='completions',
            arguments=cleared_arguments)

    @classmethod
    def completions_grouped(cls, filter: ListCompletionsFilterInput,
        group_by: CompletionGroupBy, page: CursorPageInput, order: OrderPair
        ) ->CompletionGroupDataConnectionFields:
        arguments: Dict[str, Dict[str, Any]] = {'filter': {'type':
            'ListCompletionsFilterInput!', 'value': filter}, 'groupBy': {
            'type': 'CompletionGroupBy!', 'value': group_by}, 'page': {
            'type': 'CursorPageInput!', 'value': page}, 'order': {'type':
            'OrderPair!', 'value': order}}
        cleared_arguments = {key: value for key, value in arguments.items() if
            value['value'] is not None}
        return CompletionGroupDataConnectionFields(field_name=
            'completionsGrouped', arguments=cleared_arguments)

    @classmethod
    def completion(cls, use_case: str, id: Any) ->CompletionFields:
        arguments: Dict[str, Dict[str, Any]] = {'useCase': {'type':
            'IdOrKey!', 'value': use_case}, 'id': {'type': 'UUID!', 'value':
            id}}
        cleared_arguments = {key: value for key, value in arguments.items() if
            value['value'] is not None}
        return CompletionFields(field_name='completion', arguments=
            cleared_arguments)
    ...
    def training_job(cls, id: Any) ->TrainingJobFields:
        ...
        cleared_arguments = {key: value for key, value in arguments.items() if
            value['value'] is not None}
        return TrainingJobFields(field_name='trainingJob', arguments=
            cleared_arguments)
